...
class _AdaptiveAvgPool2d(nn.Module):
    ...
    def forward(self, feats):
        fix_feats = F.interpolate(feats, size=(20,20), mode='bilinear', align_corners=False)  # 确保求出的stride和out_size适用所有图像
        in_size = 20
        out_size = self.out_size[0]
        stride =  np.floor_divide(in_size, out_size)   # int(np.floor(in_size/out_size))
        kernel_size = in_size-(out_size-1)*stride
        self.AvgCond2d = nn.AvgPool2d(kernel_size, stride)
        outs_feats = self.AvgCond2d(fix_feats)
        outs_feats = F.interpolate(outs_feats, size=[feats.size(2), feats.size(3)], mode='bilinear', align_corners=False)
        return outs_feats
...
class PyramidPoolingModule(nn.Module):
    ...
    def _make_stage(self, features, out_features, size):
        ...
        prior = _AdaptiveAvgPool2d(out_size=[size, size])
