...
class BaseIAMDecoder(nn.Module):
    ...
    @torch.no_grad()
    def _compute_coordinates(self, x):
        h, w = x.size(2), x.size(3)
        y_loc = torch.tensor([yi for yi in np.linspace(-1, 1, h)], device=x.device)
        x_loc = torch.tensor([xi for xi in np.linspace(-1, 1, w)], device=x.device)
        y_loc, x_loc = torch.meshgrid(y_loc, x_loc)
        y_loc = y_loc.expand([x.shape[0], 1, -1, -1])
        x_loc = x_loc.expand([x.shape[0], 1, -1, -1])
        locations = torch.cat([x_loc, y_loc], 1)
    ...
    def forward(self, features):
        coord_features = self._compute_coordinates(features)
